...
# Download new CSV
# This link has been edited for Github
webbrowser.open('https://docs.google.com/spreadsheets/d/e/2PACX-1vRrBJytu9qNIpDcbgQdlun1z_61dvA217sX8yz4WwbiBEpfQ6Ain5555555aNp7VgwEMrEUmX23VBxK353ZGi/pub?gid=1199397185&single=true&output=csv')
...
def open_form(fname, lname, email, phone, mycaa, program, saved_token, last_token):
    ...
    browser.get('https://xxxxxxxxxxxx.net/admin/login.html')

    # Login to the form
    button = browser.find_element_by_id('login')
    button.click()

    # Credentials edited for Github
    WebDriverWait(browser, 10).until(EC.element_to_be_clickable(EMAILFIELD)).send_keys("roddy.ezell@xxxxx.net")
    WebDriverWait(browser, 10).until(EC.element_to_be_clickable(NEXTBUTTON)).click()
    WebDriverWait(browser, 10).until(EC.element_to_be_clickable(PASSWORDFIELD)).send_keys("xxxxxxx")
    WebDriverWait(browser, 10).until(EC.element_to_be_clickable(NEXTBUTTON)).click()
    ...
    for i in range(saved_token+1, last_token+1, 1):
        ...
        if mycaa[i] == 'TRUE':
            ...
            browser.get('https://xxxxxxxxxxx.net/admin/ec-submission.html')
